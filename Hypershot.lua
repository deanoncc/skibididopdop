-- Trident Survival Aimbot and ESP Script with R15 Chams, 3D Box, and Health Display (Optimized with Manual Cache - V5 Adapted)
-- Place in: C:\Users\USER\AppData\Roaming\Assembly\Scripts

--[[
    MODIFICATION NOTES:
    - Players and local player are located in Workspace as models; display their actual names in ESP.
    - Entities in Workspace > Mobs folder are NPCs; display "NPC" instead of model name in ESP.
    - Excludes local player from ESP and aimbot targeting by matching model in Workspace with globals.localplayer().
    - Removed Key ESP and Backpack ESP (checkboxes, caching, and rendering logic).
    - Combined esp_color for all ESP modes (players and NPCs).
    ...
    - Added usernames (or "NPC") above 3D box/Dot/Skeleton ESP and distance (in meters, no decimals) below using esp_color.
    - Added health display to the left of Player/NPC ESP (Dot, Skeleton, 3D Box) from Humanoid:Health().
    - Changed health display format to "current_hp/max_hp" (e.g., "100/100") using Humanoid:Health() and Humanoid:MaxHealth().
    - Fixed nil index error by deferring local_player_name initialization to update_target_cache.
]]
...
local esp_enabled = ui.new_checkbox("Enable ESP")
local esp_mode = ui.new_combo("ESP Mode", {"Dot ESP", "Skeleton ESP", "3D Box ESP"})
...
-- Cached Data & State
local cached_targets = {} -- Players and NPCs
local current_target_id = nil 
local last_cache_update = 0
local CACHE_UPDATE_INTERVAL = 1.0 
local local_player_name = "" -- Initialized in update_target_cache
local local_player_pos = nil
local local_player_id = nil -- Store local player's model address for exclusion
...
function update_target_cache()
    ...
    if local_player then
        local local_model = local_player:ModelInstance()
        if local_model then
            local_player_name = local_model:Name()
            local_player_id = local_model:Address()
    ...
    -- Cache NPCs (Mobs folder)
    local mobs_folder = workspace:FindChild("Mobs")
    local found_this_scan = {}
    if mobs_folder then
        for _, model in ipairs(mobs_folder:Children()) do
            if model:ClassName() == "Model" and model:Address() ~= local_player_id then
                ...
                if hrp and humanoid then
                    ...
                    if not cached_targets[model_id] then
                        ...
                        if limbs["Head"] and limbs["HumanoidRootPart"] then
                            cached_targets[model_id] = { model = model, head = limbs["Head"], hrp = limbs["HumanoidRootPart"], limbs = limbs, name = "NPC", humanoid = humanoid, is_npc = true }
                        end
                    end
                end
            end
    ...
    -- Cache Players (Workspace)
    for _, model in ipairs(workspace:Children()) do
        if model:ClassName() == "Model" and model:Address() ~= local_player_id then
            local hrp = model:FindChild("HumanoidRootPart")
            local humanoid = model:FindChild("Humanoid")
            if hrp and humanoid then
                ...
                if not cached_targets[model_id] then
                    ...
                    if limbs["Head"] and limbs["HumanoidRootPart"] then
                        cached_targets[model_id] = { model = model, head = limbs["Head"], hrp = limbs["HumanoidRootPart"], limbs = limbs, name = model:Name(), humanoid = humanoid, is_npc = false }
                    end
                end
            end
        end
...
local function draw_3d_box(target_data)
    ...
    if all_corners_valid then
        ...
        -- Draw username or "NPC" above box and distance below using ESP color
        local text_pos_x = (screen_corners[1].x + screen_corners[2].x) / 2
        local display_name = target_data.is_npc and "NPC" or target_data.name
        local text_size = render.measure_text(display_name, 2)
        render.text(text_pos_x - text_size.x / 2, min_y - 15, display_name, r, g, b, a, "o", 2)
    ...
    for id, target_data in pairs(cached_targets) do
        ...
        if esp_enabled:get() then
            if selected_mode == 0 then -- Dot ESP
                ...
                if head_pos then
                    ...
                    if head_screen_pos and head_screen_pos.x > 0 and head_screen_pos.x < screen_size.x and head_screen_pos.y > 0 and head_screen_pos.y < screen_size.y then
                        ...
                        -- Draw username or "NPC" above dot and distance below HRP using ESP color
                        local display_name = target_data.is_npc and "NPC" or target_data.name
                        local text_size = render.measure_text(display_name, 2)
                        render.text(head_screen_pos.x - text_size.x / 2, head_screen_pos.y - 15, display_name, r, g, b, a, "o", 2)
            ...
            elseif selected_mode == 1 then -- Skeleton ESP
                ...
                -- Draw username or "NPC" above head and distance below HRP using ESP color
                local head_pos = target_data.head:Primitive():GetPartPosition()
                if head_pos then
                    local head_screen_pos = utils.world_to_screen(head_pos)
                    if head_screen_pos and head_screen_pos.x > 0 and head_screen_pos.x < screen_size.x and head_screen_pos.y > 0 and head_screen_pos.y < screen_size.y then
