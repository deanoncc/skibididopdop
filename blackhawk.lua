...
--[[
    MODIFICATION NOTES:
    - Uses Workspace directly for player models named "Male".
    - Excludes local player by checking for both BillboardGui (named "BillboardGui") and AnimationController (named "AnimationController") in "Male" models.
    ...
    - Removed Humanoid checks and health display (no Humanoid instance in Male models).
    - Removed reliance on localplayer:ModelInstance() for local player position; instead, finds local player's "Male" model in Workspace using BillboardGui and AnimationController.
    ...
    - Fixed nil indexing error in update_target_cache with additional checks and debug prints.
    - Updated HumanoidRootPart to Root for player model part naming.
]]
...
-- Cached Data & State
local cached_targets = {} -- Players (Models named "Male")
...
local r15_limbs_names = {
    ...
    "RightUpperLeg", "RightLowerLeg", "RightFoot", "Root"
}
...
function update_target_cache()
    ...
    -- Cache Players (Models named "Male" in Workspace, excluding local player)
    local found_this_scan = {}
    local children = workspace:Children()
    if not children then
        print("Error: workspace:Children() returned nil")
        return
    end
    ...
    if local_player then
        for _, model in ipairs(children) do
            if not model then
                print("Error: Model in workspace:Children() is nil")
                goto continue_local
            end
            local class_name = model:ClassName()
            local model_name = model:Name()
            if class_name == "Model" and model_name == "Male" then
                local billboard_gui = model:FindChild("BillboardGui")
                local animation_controller = model:FindChild("AnimationController")
                if billboard_gui and animation_controller then
                    local_root = model:FindChild("Root")
                    break
                end
            end
            ::continue_local::
        ...
        if local_root then
            local local_root_primitive = local_root:Primitive()
            if local_root_primitive then
                local_player_pos = local_root_primitive:GetPartPosition()
            else
                local_player_pos = nil
                print("Error: Local player Root primitive is nil")
            end
        else
            local_player_pos = nil
            print("Error: Local player missing Root or not found in Male models with BillboardGui and AnimationController")
        end
    else
        local_player_pos = nil
    ...
    for _, model in ipairs(children) do
        ...
        local model_name = model:Name()
        if class_name == "Model" and model_name == "Male" then
            local root = model:FindChild("Root")
            local billboard_gui = model:FindChild("BillboardGui")
            local animation_controller = model:FindChild("AnimationController")
            -- Exclude local player if model has both BillboardGui and AnimationController
            if root and not (billboard_gui and animation_controller) then
                local model_id = model:Address()
                if not model_id then
                    print("Error: model:Address() returned nil for model " .. model_name)
                    goto continue
                end
                ...
                if not cached_targets[model_id] then
                    ...
                    if limbs["Head"] and limbs["Root"] then
                        cached_targets[model_id] = { 
                            model = model, 
                            head = limbs["Head"], 
                            hrp = limbs["Root"], 
                            limbs = limbs, 
                        ...
                        print("Error: Missing Head or Root for model " .. model_name)
                    end
                end
            else
                if not root then
                    print("Error: Missing Root for model " .. model_name)
                end
            end
        end
        ::continue::
    ...
    for id, target_data in pairs(cached_targets) do
        ...
        if esp_enabled:get() then
            if selected_mode == 0 then -- Dot ESP
                ...
                if head_pos then
                    ...
                    if head_screen_pos and head_screen_pos.x > 0 and head_screen_pos.x < screen_size.x and head_screen_pos.y > 0 and head_screen_pos.y < screen_size.y then
                        ...
                        -- Draw username above dot and distance below Root using ESP color
                        local username = target_data.name
                        local text_size = render.measure_text(username, 2)
            ...
            elseif selected_mode == 1 then -- Skeleton ESP
                ...
                -- Draw username above head and distance below Root using ESP color
                local head_pos = target_data.head:Primitive():GetPartPosition()
                if head_pos then
                    local head_screen_pos = utils.world_to_screen(head_pos)
                    if head_screen_pos and head_screen_pos.x > 0 and head_screen_pos.x < screen_size.x and head_screen_pos.y > 0 and head_screen_pos.y < screen_size.y then
...
cheat.set_callback("shutdown", function()
    ...
    print("Trident Survival (Male Models, V5) script unloaded cleanly.")
end)

print("blak howk loadid")