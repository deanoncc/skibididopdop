...
--[[
    ...
    - Combined esp_color for all ESP modes (players).
    - Removed Look Vector ESP, Radar ESP, velocity prediction, bullet drop compensation, and health check.
    - Maintains V3 optimizations: stable ESP visibility.
    - Fixed typo in draw_3d_box (sole to local for step_thickness, screenPos to screen_pos).
    - Added usernames above 3D box/Dot/Skeleton ESP and distance (in meters, no decimals) below using esp_color.
    - Removed Items ESP, Keys ESP, and Backpacks ESP (checkboxes and functionality).
    - Removed health display from Player ESP.
    - Updated Skeleton ESP to handle custom limbs (MeshParts and Parts) instead of fixed R15 limb names.
    - Optimized Skeleton ESP to use minimal limbs (Head, HumanoidRootPart, TorsoUpperFront, TorsoPelvis, Left/RightArmUpper/Lower, Left/RightLegUpper/Lower) for a clean skeleton effect.
    - Removed Holding Weapon ESP (checkbox and functionality).
]]
            ...
            if model:ClassName() == "Model" and model:Name() ~= local_player_name then
                ...
                if hrp and humanoid then
                    ...
                    if not cached_targets[model_id] then
                        ...
                        local left_arm_folder = model:FindChild("LeftArm")
                        if left_arm_folder then
                            for _, part in ipairs(left_arm_folder:Children()) do
                                if part:ClassName() == "MeshPart" or part:ClassName() == "Part" then
                                    limbs[part:Name()] = part
                                end
                            end
                        end
                        local right_arm_folder = model:FindChild("RightArm")
                        if right_arm_folder then
                            for _, part in ipairs(right_arm_folder:Children()) do
                                if part:ClassName() == "MeshPart" or part:ClassName() == "Part" then
                                    limbs[part:Name()] = part
                                end
                            end
                        end
                        local left_leg_folder = model:FindChild("LeftLeg")
                        if left_leg_folder then
                            for _, part in ipairs(left_leg_folder:Children()) do
                                if part:ClassName() == "MeshPart" or part:ClassName() == "Part" then
                                    limbs[part:Name()] = part
                                end
                            end
                        end
                        local right_leg_folder = model:FindChild("RightLeg")
                        if right_leg_folder then
                            for _, part in ipairs(right_leg_folder:Children()) do
                                if part:ClassName() == "MeshPart" or part:ClassName() == "Part" then
                                    limbs[part:Name()] = part
                                end
                            end
...
local function draw_limb_chams(limbs_table)
    ...
    local required_limbs = {
        "Head", "HumanoidRootPart", "TorsoUpperFront", "TorsoPelvis",
        "LeftArmUpper", "LeftArmLower", "RightArmUpper", "RightArmLower",
        "LeftLegUpper", "LeftLegLower", "RightLegUpper", "RightLegLower"
    }
    for _, name in ipairs(required_limbs) do
        local part = limbs_table[name]
        if part then
